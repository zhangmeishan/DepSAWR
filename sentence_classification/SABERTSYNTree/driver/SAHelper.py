...
class SentenceClassifier(object):
    ...
    def forward(self, inputs, rels, heads, masks, lengths):
        ...
        tag_logits = self.model.forward(bert_outputs, rels, heads, masks, lengths)
        # cache
    ...
    def classifier(self, inputs, rels, heads, masks, lengths):
        if inputs is not None:
            self.forward(inputs, rels, heads, masks, lengths)
        pred_tags = self.tag_logits.detach().max(1)[1].cpu()
        return pred_tags
