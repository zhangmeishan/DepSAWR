...
def train(data, dev_data, test_data, classifier, vocab, dep_vocab, config):
    ...
    if config.parser_tune == 1:
        optimizer_parser = Optimizer(filter(lambda p: p.requires_grad, classifier.parser.parameters()), config)
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            bert_inputs, tags, masks, dep_words, dep_extwords, dep_masks = \
                batch_data_variable(onebatch, vocab, dep_vocab)

            classifier.model.train()
            classifier.forward(bert_inputs, masks, dep_words, dep_extwords, dep_masks)
            ...
            if batch_iter % config.update_every == 0 or batch_iter == batch_num:
                ...
                if config.parser_tune == 1:
                    nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, classifier.parser.parameters()), \
                                            max_norm=config.clip)
...
def evaluate(data, classifier, vocab, dep_vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        bert_inputs, tags, masks, dep_words, dep_extwords, dep_masks = \
            batch_data_variable(onebatch, vocab, dep_vocab)
        count = 0
        pred_tags = classifier.classifier(bert_inputs, masks, dep_words, dep_extwords, dep_masks)
...
if __name__ == '__main__':
    ...
    model = SAModel(vocab, config, parser_config, bert.bert_hidden_size, bert.bert_layers)
    ...
    classifier = SentenceClassifier(model, bert, vocab, parser_model, dep_vocab)
