...
class Encoder(nn.Module):
    def __init__(
            self, n_src_vocab, n_rels, n_layers=6, n_head=8,
    ...
    def forward(self, src_seq, rels, heads, xlengths):
        ...
        enc_mask = src_seq.data.eq(SRCVocab.PAD)
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.embeddings = Embeddings(n_tgt_vocab, d_word_vec, dropout=dropout,
                                     add_position_embedding=True, padding_idx=TGTVocab.PAD)
    ...
    def forward(self, tgt_seq, enc_output, enc_mask, enc_attn_caches=None, self_attn_caches=None):
        ...
        # Decode mask
        dec_slf_attn_pad_mask = tgt_seq.data.eq(TGTVocab.PAD).unsqueeze(1).expand(batch_size, query_len, key_len)
...
class Transformer(nn.Module):
    ...
    def __init__(
            self, config, n_src_vocab, n_rels, n_tgt_vocab, use_gpu=True):

        super(Transformer, self).__init__()

        self.encoder = Encoder(n_src_vocab, n_rels,
        ...
        if config.proj_share_weight:
            ...
            self.generator = Generator(n_words=n_tgt_vocab, hidden_size=config.embed_size, padding_idx=TGTVocab.PAD)
    ...
    def force_teaching(self, x, rel, head, y, lengths):
        enc_output, enc_mask = self.encoder(x, rel, head, lengths)
        dec_output, _, _ = self.decoder(y, enc_output, enc_mask)
        return dec_output

    def batch_beam_search(self, x, rel, head, lengths, beam_size=5, max_steps=150):
        batch_size = x.size(0)

        enc_output, enc_mask = self.encoder(x, rel, head, lengths) # [batch_size, seq_len, dim]
        ...
        final_word_indices = x.data.new(batch_size, beam_size, 1).fill_(TGTVocab.BOS) # Word indices in the beam
