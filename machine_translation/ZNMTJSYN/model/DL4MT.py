...
class Encoder(nn.Module):
    ...
    def forward(self, x, synxs, xlengths):
        """
...
class DL4MT(nn.Module):
    def __init__(self, config, p_config, n_src_vocab, n_tgt_vocab, use_gpu=True):

        super().__init__()

        self.config = config

        self.encoder = Encoder(parser_config=p_config, n_words=n_src_vocab, input_size=config.embed_size, \
