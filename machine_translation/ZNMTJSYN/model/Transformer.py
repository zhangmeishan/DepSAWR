...
class Encoder(nn.Module):
    def __init__(
            self, parser_config, n_src_vocab, n_layers=6, n_head=8,
        ...
        for layer in range(parser_config.lstm_layers):
            transformer_lstm.append(nn.Linear(parser_dim, d_word_vec, bias=False))
        self.transformer_lstm = nn.ModuleList(transformer_lstm)

        parser_mlp_dim = parser_config.mlp_arc_size + parser_config.mlp_rel_size
        self.transformer_dep = nn.Linear(parser_mlp_dim, d_word_vec, bias=False)
        self.transformer_head = nn.Linear(parser_mlp_dim, d_word_vec, bias=False)

        self.parser_lstm_layers = parser_config.lstm_layers
        self.synscale = ScalarMix(mixture_size=3+parser_config.lstm_layers)
        self.transformer = nn.Linear(d_word_vec + d_word_vec, d_model, bias=False)
    ...
    def forward(self, src_seq, synxs):
        ...
        out = self.transformer(encoder_input)
...
class Transformer(nn.Module):
    ...
    def __init__(
            self, config, p_config, n_src_vocab, n_tgt_vocab, use_gpu=True):

        super(Transformer, self).__init__()

        self.encoder = Encoder(p_config, n_src_vocab,
            n_layers=config.num_layers, n_head=config.num_heads,
    ...
    def forward(self, src_seq, src_synxs, tgt_seq=None, mode="train", **kwargs):
        if mode == "train":
            ...
            return self.batch_beam_search(src_seq=src_seq, synxs=src_synxs, **kwargs)

    def force_teaching(self, src_seq, src_synxs, tgt_seq, lengths):
        enc_output, enc_mask = self.encoder(src_seq, src_synxs)
        dec_output, _, _ = self.decoder(tgt_seq, enc_output, enc_mask)

        return dec_output

    def batch_beam_search(self, src_seq, synxs, lengths, beam_size=5, max_steps=150):

        batch_size = src_seq.size(0)

        enc_output, enc_mask = self.encoder(src_seq, synxs) # [batch_size, seq_len, dim]
