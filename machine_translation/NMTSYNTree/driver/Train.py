...
if __name__ == '__main__':
    ...
    rel_vocab = RELVocab(config.rel_vocab_path)
    tgt_vocab = NMTVocab(config.tgt_vocab_type, config.tgt_vocab_path)

    print("Sentence Number: #train = %d" % (len(train_srcs)))

    # model
    nmt_model = eval(config.model_name)(config, src_vocab, rel_vocab, tgt_vocab, config.use_cuda)
    ...
    nmt = NMTHelper(nmt_model, critic, src_vocab, rel_vocab, tgt_vocab, config)
