...
class Vocab(object):
    ...
    def __init__(self, word_counter, rel_counter, tag_counter, bert_vocab_file, min_occur_count=1):
        self._id2word = ['<pad>', '<bos>', '<eos>', '<unk>']
        self._wordid2freq = [10000, 10000, 10000, 10000]
        self._id2rel = ['<pad>', '<bos>', '<eos>', '<unk>']
        self._id2tag = []
        ...
        print("Vocab info: #words %d, #rels %d, #tags %d" % (self.vocab_size, self.rel_size, self.tag_size))
