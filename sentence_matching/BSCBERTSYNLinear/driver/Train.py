...
def train(data, dev_data, test_data, bisent_classfier, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            tinst, src_indexes, tgt_indexes = batch_data_variable(onebatch, vocab)
            bisent_classfier.model.train()
            if bisent_classfier.use_cuda:
                tinst.to_cuda(bisent_classfier.device)
            bisent_classfier.forward(tinst.inputs, src_indexes, tgt_indexes)
            ...
            if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                tag_correct, tag_total, dev_tag_acc = \
                    evaluate(dev_data, bisent_classfier, vocab, config.dev_file + '.' + str(global_step))
                print("Dev: acc = %d/%d = %.2f, lr = %.8f" % (tag_correct, tag_total, dev_tag_acc, optimizer_model.lr))

                tag_correct, tag_total, test_tag_acc = \
                    evaluate(test_data, bisent_classfier, vocab, config.test_file + '.' + str(global_step))
...
def evaluate(data, bisent_classfier, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        ...
        if bisent_classfier.use_cuda:
            tinst.to_cuda(bisent_classfier.device)
        count = 0
        pred_tags = bisent_classfier.classifier(tinst.inputs, src_indexes, tgt_indexes)
