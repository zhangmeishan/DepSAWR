...
class Vocab(object):
    ...
    def __init__(self, word_counter, action_counter, tag_counter, bert_vocab_file, min_occur_count=1):
        self._id2word = ['<pad>', '<bos>', '<eos>', '<unk>']
        self._wordid2freq = [10000, 10000, 10000, 10000]
        self._id2action = ['<pad>', '<bos>', '<eos>', '<unk>']
        self._id2tag = []
        ...
        self._action2id = reverse(self._id2action)
        if len(self._action2id) != len(self._id2action):
        ...
        print("Vocab info: #words %d, #action %d, #tags %d" % (self.vocab_size, self.action_size, self.tag_size))
    ...
    def action2id(self, xs):
        if isinstance(xs, list):
            return [self._action2id.get(x, self.PAD) for x in xs]
        return self._action2id.get(xs, self.PAD)

    def id2action(self, xs):
        if isinstance(xs, list):
            return [self._id2action[x] for x in xs]
        return self._id2action[xs]
