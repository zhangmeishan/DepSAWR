...
def creatVocab(corpusFile, min_occur_count):
    ...
    for inst in alldatas:
        ...
        for curword in inst.tgt_forms:
            word_counter[curword] += 1
        for currel in inst.src_rels:
            rel_counter[currel] += 1
        for currel in inst.tgt_rels:
            rel_counter[currel] += 1
        tag_counter[inst.tag] += 1

    return Vocab(word_counter, rel_counter, tag_counter, min_occur_count)
...
def inst2id(inst, vocab):
    ...
    src_relids = vocab.rel2id(inst.src_rels)
    tgt_relids = vocab.rel2id(inst.tgt_rels)
    tagid = vocab.tag2id(inst.tag)

    return src_ids, src_extids, src_relids, tgt_ids, tgt_extids, tgt_relids, tagid, inst
...
def batch_data_variable(batch, vocab):
    ...
    b = 0
    for src_ids, src_extids, src_relids, tgt_ids, tgt_extids, tgt_relids, tagid, inst \
