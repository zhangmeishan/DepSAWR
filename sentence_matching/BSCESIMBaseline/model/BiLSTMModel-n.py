...
class BiLSTMModel(nn.Module):
    def __init__(self, vocab, config, init_embedding):
        ...
        self.feature_dim = 4 * 2 * config.lstm_hiddens
    ...
    def forward(self, tinputs):
        ...
        src_hidden_avg = torch.sum(src_hiddens * src_masks.unsqueeze(1)
                                   .transpose(2, 1), dim=1) \
                         / (torch.sum(src_masks, dim=1, keepdim=True) + 1e-7)

        tgt_hidden_avg = torch.sum(tgt_hiddens * tgt_masks.unsqueeze(1)
                                   .transpose(2, 1), dim=1) \
                         / (torch.sum(tgt_masks, dim=1, keepdim=True) + 1e-7)

        src_tgt_diff = torch.abs(src_hidden_avg - tgt_hidden_avg)
        src_tgt_dot = src_hidden_avg * tgt_hidden_avg

        hiddens = torch.cat([src_hidden_avg, tgt_hidden_avg, src_tgt_diff, src_tgt_dot], dim=1)
