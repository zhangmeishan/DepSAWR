...
class TensorInstances:
    def __init__(self, batch_size, slen, tlen):
        ...
        self.src_rels = Variable(torch.LongTensor(batch_size, slen).zero_(), requires_grad=False)
        ...
        self.tgt_rels = Variable(torch.LongTensor(batch_size, tlen).zero_(), requires_grad=False)
    ...
    @property
    def inputs(self):
        return (self.src_words, self.src_extwords, self.src_rels, self.src_heads, \
                self.src_lens, self.src_masks, \
                self.tgt_words, self.tgt_extwords, self.tgt_rels, self.tgt_heads, \
