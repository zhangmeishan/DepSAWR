...
class CRF(nn.Module):
    ...
    def forward(self,
        ...
        Returns
        -------
        :class:`~torch.autograd.torch.Tensor`
        ...
        if mask is None:
            mask = torch.ones(*tags.size(), dtype=torch.float)
    ...
    def decode(self,
               ...
               mask: Optional[Union[torch.Tensor, torch.Tensor]] = None) -> List[List[int]]:
        """Find the most likely tag sequence using Viterbi algorithm.

        Arguments
        ---------
        emissions : :class:`~torch.autograd.torch.Tensor` or :class:`~torch.FloatTensor`
            Emission score tensor of size ``(seq_length, batch_size, num_tags)``.
        mask : :class:`~torch.autograd.torch.Tensor` or :class:`torch.Tensor`
    ...
    def _viterbi_decode(self, emissions: torch.FloatTensor, mask: torch.Tensor) \
            -> List[List[int]]:
        # Get input sizes
        seq_length = emissions.size(0)
