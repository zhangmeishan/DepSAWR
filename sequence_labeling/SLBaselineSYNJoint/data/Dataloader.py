...
def inst2id(inst, vocab, dep_vocab):
    ...
    for curword in inst.words:
        ...
        if index < inst.key_start or index > inst.key_end:
            ...
            labelid = vocab.PAD
        index = index + 1
        inputs.append([wordid, extwordid, dep_wordid, dep_extwordid, labelid])
...
def batch_data_variable(batch, vocab, dep_vocab):
    ...
    b = 0
    for inputs, key_start, key_end in insts_numberize(batch, vocab, dep_vocab):
        index = 0
    ...
    return words, extwords, predicts, masks, \
           dep_words, dep_extwords, dep_masks, labels
