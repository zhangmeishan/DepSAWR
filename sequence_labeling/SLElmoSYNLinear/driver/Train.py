...
def train(data, dev_data, test_data, labeler, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            words, actions, predicts, masks, word_indexes, indices, wmasks, labels = \
                batch_data_variable(onebatch, vocab)

            labeler.model.train()

            labeler.forward(words, actions, predicts, masks, word_indexes, indices)
            loss = labeler.compute_loss(labels, wmasks)
...
def evaluate(data, labeler, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, actions, predicts, masks, word_indexes, indices, wmasks, labels = \
            batch_data_variable(onebatch, vocab)
        count = 0
        predict_labels = labeler.labeler(words, actions, predicts, masks,
                                         word_indexes, indices, wmasks)
...
if __name__ == '__main__':
    ...
    labeler = SequenceLabeler(model, elmo)
