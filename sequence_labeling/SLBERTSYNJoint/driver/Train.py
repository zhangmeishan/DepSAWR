...
def train(data, dev_data, test_data, labeler, vocab, dep_vocab, config):
    ...
    if config.parser_tune == 1:
        optimizer_parser = Optimizer(filter(lambda p: p.requires_grad, labeler.parser.parameters()), config)
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            bert_inputs, predicts, masks, dep_words, dep_extwords, dep_masks, labels = \
                batch_data_variable(onebatch, vocab, dep_vocab)

            labeler.model.train()

            labeler.forward(bert_inputs, predicts, masks, dep_words, dep_extwords, dep_masks)
            ...
            if batch_iter % config.update_every == 0 or batch_iter == batch_num:
                ...
                if config.parser_tune == 1:
                    nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, labeler.parser.parameters()), \
                                            max_norm=config.clip)
...
def evaluate(data, labeler, vocab, dep_vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        bert_inputs, predicts, masks, dep_words, dep_extwords, dep_masks, labels = \
            batch_data_variable(onebatch, vocab, dep_vocab)
        count = 0
        predict_labels = labeler.labeler(bert_inputs, predicts, masks, dep_words, dep_extwords, dep_masks)
