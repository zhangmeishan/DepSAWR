...
def creat_vocab(corpusFile, bert_vocab_file, min_occur_count):
    ...
    return SLVocab(word_counter, rel_counter, label_counter, bert_vocab_file, min_occur_count)
...
def inst2id(inst, vocab):
    ...
    for curword in inst.words:
        ...
        if index < inst.key_start or index > inst.key_end:
            ...
            labelid = vocab.PAD
        index = index + 1
        inputs.append([relid, curword.head, labelid])
...
def batch_data_variable(batch, vocab):
    ...
    for inputs, key_start, key_end, inst in insts_numberize(batch, vocab):
        ...
        for curword in inputs:
            masks[b, index] = 1
            predicts[b, index] = 2
            rels[b, index] = curword[0]
            cur_heads.append(curword[1])
    ...
    return bert_inputs, predicts, masks, rels, heads, lengths, labels
