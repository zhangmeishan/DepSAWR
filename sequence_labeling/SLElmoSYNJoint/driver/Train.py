...
def train(data, dev_data, test_data, labeler, vocab, dep_vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            words, predicts, masks, dep_words, dep_extwords, dep_masks, labels = \
                batch_data_variable(onebatch, vocab, dep_vocab)

            labeler.model.train()

            labeler.forward(words, predicts, masks, dep_words, dep_extwords, dep_masks)
...
def evaluate(data, labeler, vocab, dep_vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, predicts, masks, dep_words, dep_extwords, dep_masks, labels = \
            batch_data_variable(onebatch, vocab, dep_vocab)
        count = 0
        predict_labels = labeler.labeler(words, predicts, masks, dep_words, dep_extwords, dep_masks)
...
if __name__ == '__main__':
    ...
    labeler = SequenceLabeler(model, elmo, parser_model)
