class Word:
    def __init__(self, id, form, rel, head, label):
        self.id = id
        self.org_form = form
    ...
    def __str__(self):
        values = [str(self.id), self.org_form, self.rel, self.head, self.label]
        return '\t'.join(values)
...
def readInstance(file):
    ...
    for line in file:
        ...
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            ...
            try:
                words.append(Word(int(tok[0]), tok[1], tok[2], int(tok[3]), tok[4]))
            except Exception:
                pass
        else:
            pass
