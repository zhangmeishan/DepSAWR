...
def creat_vocab(corpusFile, min_occur_count):
    ...
    return SLVocab(word_counter, rel_counter, label_counter, min_occur_count)
...
def inst2id(inst, vocab):
    ...
    for curword in inst.words:
        ...
        if index < inst.key_start or index > inst.key_end:
            ...
            labelid = vocab.PAD
        index = index + 1
        inputs.append([wordid, relid, curword.head, labelid])
...
def batch_data_variable(batch, vocab):
    ...
    for inputs, key_start, key_end, inst in insts_numberize(batch, vocab):
        ...
        for curword in inputs:
            ...
            cur_heads.append(curword[2])
            labels[b, index] = curword[3]
    ...
    return words, predicts, masks, rels, heads, lengths, labels
